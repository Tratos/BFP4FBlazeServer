    ...
    public static class AuthenticationComponent
    {
        ...
        public static void Login(Blaze.Packet p, PlayerInfo pi, NetworkStream ns)
        {
            if (!pi.isServer)
            {
                ...
                if (pi.profile == null)
                {
                    Logger.Error("[CLNT] #" + pi.userId + " Could not find player profile for token 0x" + id.ToString("X") + "!");
                    pi.userId = 0;
                    return;
                }
                else
                {
                    ...
                    pi.userId = id;
                    Logger.Data("[CLNT] New ID #" + pi.userId + " Client Playername = \"" + pi.profile.name + "\"");
                }
